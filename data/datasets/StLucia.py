...
def get_query_paths(session_type='ms'):
    return sorted(glob.glob(config.root_dir + '/data/raw_data/StLucia_small/180809_1545/*.jpg'))


def get_map_paths(session_type='ms'):
    return sorted(glob.glob(config.root_dir + '/data/raw_data/StLucia_small/100909_0845/*.jpg'))

def get_gtmatrix(session_type='ms', gt_type='soft'):
    gt_data = np.load(config.root_dir + '/data/raw_data/StLucia_small/GT.npz')
    if gt_type=='hard':
...
def download():
    destination = config.root_dir + '/data/raw_data/StLucia_small'
